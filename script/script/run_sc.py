...
def run_cellranger(sample_info,data_dir,species):
    ...
    genome_config="/Business/psn_company/sc01/Workbase_Wangcanbiao/script/ref.json"
...
def run_step_analysis(sample_info_dic,sample_type,mt_type,mt_cutoff):
    ...
    shutil.copyfile("/Business/psn_company/sc01/Workbase_Wangcanbiao/script/seurat.R","summary/seurat.R")
    cmd="cd summary/;bsub -q psnpublic -I  -e seurat.err -o seurat.log /Business/psn_company/sc01/software/R-4.0.5/bin/Rscript seurat.R -t {} -m {} -f {}".format(sample_type,mt_type,mt_cutoff)
    run_cmds.write("{}\n".format(cmd))
    execCmd(cmd)

def format_file():
    cmd="cp /Business/psn_company/sc01/Workbase_Wangcanbiao/script/format.sh . && bash format.sh"
    execCmd(cmd)
