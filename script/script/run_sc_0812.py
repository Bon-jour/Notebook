...
def runQC(data_dir):
    ...
    cmd="mkdir {}/QC && cd {} && bsub -q psnpublic -e qc.log -e qc.err /Business/psn_company/t01/local/bin/fastq -j /Business/psn_company/t01/local/jdk1.8.0_172/bin/java -t 10 -o QC */*gz".format(data_dir,data_dir)
    execCmd(cmd)
...
def run_cellranger(sample_info,data_dir,species):
    ...
    for line in open(sample_file):
        if not line.startswith("sample"):
            ...
            cmd="bsub -q psnpublic -e {}.err -I -o {}.log \
...
def run_cellranger_aggr(sample_info_dic):
    ...
    for sample in sample_info_dic.keys():
        h5_dir=os.path.abspath("./summary/{}/outs/molecule_info.h5".format(sample))
...
def run_step_analysis(sample_info_dic,sample_type,mt_type,mt_cutoff):
    ...
    cmd="cd summary/;Rscript seurat.R -t {} -m {} -f {}".format(sample_type,mt_type,mt_cutoff)
    run_cmds.write("{}\n".format(cmd))
    execCmd(cmd)
