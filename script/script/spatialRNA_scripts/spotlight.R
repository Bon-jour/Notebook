...
spotlight_dir=paste(project_dir,"06_SpotLight",sep="/")
...
for(i in 1:nrow(sample_list)){
  ...
  if(file.exists(paste(spotlight_dir_sample,"spotlight.finished",sep="/"))){
      next()
  }

  if(file.exists(paste(rds_dir,paste0("spotlight.",sample_list[i,1],".rds"),sep="/"))){
    mapper=c()
    spotlight[[sample_list[i,1]]]=readRDS(paste(rds_dir,paste0("spotlight.",sample_list[i,1],".rds"),sep="/"))
    for (j in 1:length(sc_data.list)){
      mapper=unique(c(mapper,levels(sc_data.list[[j]]@meta.data$celltype)))
    }
  }else{

    if(sample_list[i,5] %in% sc_sample_list){

      sc4spotlight[[sample_list[i,5]]] = SCTransform(sc_data.list[[sample_list[i,5]]], verbose = opt$verbose) %>%
      RunPCA(., verbose = opt$verbose) %>%
      RunUMAP(., dims = 1:30, verbose = opt$verbose)
      cluster_markers_all[[sample_list[i,5]]] <- FindAllMarkers(object = sc4spotlight[[sample_list[i,5]]], 
                                            assay = "SCT",
                                            slot = "data",
                                            verbose = opt$verbose, 
                                            only.pos = TRUE,             # 只选择在cluster上调的基因，only.pos=T；
                                            logfc.threshold = 0.25,      # 从SCT的data数据中获取差异基因，为了获得所有可能的差异基因，不需要太高的倍数变化；
                                            min.pct = 0.5)               # 选择在同类细胞中半数细胞都表达的基因，min.pct=0.5；如果效果不好，则继续降低

      sc4spotlight[[sample_list[i,5]]]@misc[["cluster_markers_all4spotlight"]]=cluster_markers_all[[sample_list[i,5]]]        # 储存这个数据到misc                            
      ...
      spotlight[[sample_list[i,1]]] <-  spotlight_deconvolution(
                                        se_sc = sc4spotlight[[sample_list[i,5]]], 
                                        counts_spatial = spatial.ob[[sample_list[i,1]]]@assays$Spatial@counts,
                                        clust_vr = "celltype3",                                    # Variable in sc_seu containing the cell-type annotation
                                        cluster_markers = cluster_markers_all[[sample_list[i,5]]], # Dataframe with the marker genes
                                        cl_n = 100,                                                # number of cells per cell type to use
                                        hvg = 3000,                                                # Number of HVG to use
                                        ntop = NULL,                                               # How many of the marker genes to use (by default all)
                                        transf = "uv",                                             # Perform unit-variance scaling per cell and spot prior to factorzation and NLS
                                        method = "nsNMF",                                          # Factorization method
                                        min_cont = 0                                               # Remove those cells contributing to a spot below a certain threshold 一般认为1个spot有10几个细胞
                                        )
      ...
      if(is.null(cluster_markers_all[["all"]] )){
          if(file.exists(paste(rds_dir,"sc_combined.sct.rds",sep="/"))){
              ...
              combined.sct = SCTransform(combined, verbose = opt$verbose) %>%
              RunPCA(., verbose = opt$verbose) %>%
              RunUMAP(., dims = 1:30, verbose = opt$verbose)

              cluster_markers_all[["all"]] <- FindAllMarkers(object = combined.sct, 
                                                    assay = "SCT",
                                                    slot = "data",
                                                    verbose = opt$verbose, 
                                                    only.pos = TRUE,             # 只选择在cluster上调的基因，only.pos=T；
                                                    logfc.threshold = 0.25,      # 从SCT的data数据中获取差异基因，为了获得所有可能的差异基因，不需要太高的倍数变化；
                                                    min.pct = 0.5)               # 选择在同类细胞中半数细胞都表达的基因，min.pct=0.5；如果效果不好，则继续降低
              saveRDS(combined.sct,paste(rds_dir,"sc_combined.sct.rds",sep="/"))
              # saveRDS(combined.sct,paste(rds_dir,"sc_combined.Y38_LHM.Y38_LWY.sct.rds",sep="/"))
          }
      }
                                          

      combined.sct@misc[["cluster_markers_all4spotlight"]]=cluster_markers_all[["all"]]        # 储存这个数据到misc                            

      #2. 利用单细胞数据对空间表达数据进行反卷积：
      combined.sct@meta.data$celltype3=paste0("celltype.",as.numeric(as.factor(combined.sct@meta.data$celltype)))

      mapper=levels(as.factor(combined.sct@meta.data$celltype))
      spotlight[[sample_list[i,1]]] <-  spotlight_deconvolution(
                              se_sc = combined.sct, 
                              counts_spatial = spatial.ob[[sample_list[i,1]]]@assays$Spatial@counts,
                              clust_vr = "celltype3",                                    # Variable in sc_seu containing the cell-type annotation
                              cluster_markers = cluster_markers_all[["all"]],            # Dataframe with the marker genes
                              cl_n = 100,                                               # number of cells per cell type to use
                              hvg = 3000,                                                # Number of HVG to use
                              ntop = NULL,                                               # How many of the marker genes to use (by default all)
                              transf = "uv",                                             # Perform unit-variance scaling per cell and spot prior to factorzation and NLS
    ...
    saveRDS(spotlight[[sample_list[i,1]]],paste(rds_dir,paste0("spotlight.",sample_list[i,1],".rds"),sep="/"))
    # saveRDS(spotlight[[sample_list[i,1]]],paste(rds_dir,paste0("spotlight.Y38_LHM.Y38_LWY.",sample_list[i,1],".rds"),sep="/"))
  }

  nmf_mod[[sample_list[i,1]]] <- spotlight[[sample_list[i,1]]][[1]]
  nmf_mod[[sample_list[i,1]]][[2]]=mapper[as.numeric(gsub("celltype.","",nmf_mod[[sample_list[i,1]]][[2]]))]
  decon_mtrx[[sample_list[i,1]]] <-  spotlight[[sample_list[i,1]]][[2]]
  colnames(decon_mtrx[[sample_list[i,1]]])[-ncol(decon_mtrx[[sample_list[i,1]]])] = paste0(mapper[as.numeric(gsub("celltype.","",colnames(decon_mtrx[[sample_list[i,1]]])[-ncol(decon_mtrx[[sample_list[i,1]]])]))],"_raw")
  cell_types_all[[sample_list[i,1]]] <- colnames(decon_mtrx[[sample_list[i,1]]])[which(colnames(decon_mtrx[[sample_list[i,1]]]) != "res_ss")]

  ##将信息添加到空间数据中

  #spatial.ob[[sample_list[i,1]]]@meta.data <- cbind(spatial.ob[[sample_list[i,1]]]@meta.data, decon_mtrx[[sample_list[i,1]]])
  
  decon_df<- decon_mtrx[[sample_list[i,1]]] %>%
  data.frame() %>%
  ...
  ## 评估去卷积效果
  h <- NMF::coef( nmf_mod[[sample_list[i,1]]][[1]])
  rownames(h) <- paste("Topic", 1:nrow(h), sep = "_")
  topic_profile_plts <- dot_plot_profiles_fun(
                              h = h,
                              train_cell_clust = nmf_mod[[sample_list[i,1]]][[2]])

  plot1 = topic_profile_plts[[2]] + 
          theme(axis.text.x = element_text(angle = 90), 
                axis.text = element_text(size = 12))
  ggsave(plot1, width=5, height=5,dpi=300, filename=paste(spotlight_dir_sample,paste0("topic profiles_by_cell_type_",sample_list[i,1],".png"),sep="/"),limitsize = FALSE)

  plot2 = topic_profile_plts[[1]] + 
          theme(axis.text.x = element_text(angle = 90), 
                axis.text = element_text(size = 12))
  ggsave(plot2, width=10, height=10,dpi=300, filename=paste(spotlight_dir_sample,paste0("topic profiles_within_cell_type_",sample_list[i,1],".png"),sep="/"),limitsize = FALSE)

  basis_spotlight <- data.frame(NMF::basis(nmf_mod[[sample_list[i,1]]][[1]]))

  colnames(basis_spotlight) <- unique(stringr::str_wrap(nmf_mod[[sample_list[i,1]]][[2]], width = 30))
  important_genes = basis_spotlight 
  
  #过滤掉每个spot中贡献小于0.09的细胞类型 This is the equivalent to setting min_cont to 0.09
  decon_mtrx_sub[[sample_list[i,1]]] <- decon_mtrx[[sample_list[i,1]]][, colnames(decon_mtrx[[sample_list[i,1]]]) != "res_ss"]
  decon_mtrx_sub[[sample_list[i,1]]][decon_mtrx_sub[[sample_list[i,1]]] < 0.18] <- 0
  colnames(decon_mtrx_sub[[sample_list[i,1]]])=colnames(decon_mtrx[[sample_list[i,1]]])[-ncol(decon_mtrx[[sample_list[i,1]]])] 
  colnames( decon_mtrx_sub[[sample_list[i,1]]])=gsub("_raw","",colnames(decon_mtrx_sub[[sample_list[i,1]]]))
  rownames(decon_mtrx_sub[[sample_list[i,1]]]) <- colnames(spatial.ob[[sample_list[i,1]]])

  decon_df<- decon_mtrx_sub[[sample_list[i,1]]] %>%
  data.frame() %>%
  tibble::rownames_to_column("barcodes")
  colnames( decon_df)[which(colnames( decon_df)!="barcodes")]=colnames( decon_mtrx_sub[[sample_list[i,1]]],)
  ...
  plot5=spatial_scatterpie(se_obj = spatial.ob[[sample_list[i,1]]],
                              cell_types_all = gsub("_raw","",cell_types_all[[sample_list[i,1]]]),
  ...
  ggsave(plot5, width=5, height=5,dpi=600,units ="in",bg="white", filename= paste(spotlight_dir_sample,paste0("spatial_scatterpie_",sample_list[i,1],".png"),sep="/"),limitsize = FALSE)
  ...
  for(j in 1:length( cell_types_all[[sample_list[i,1]]])){
    cell=gsub("_raw","",cell_types_all[[sample_list[i,1]]][j])
    ...
    plot4= spatial_scatterpie(se_obj = spatial.ob[[sample_list[i,1]]],
                              cell_types_all = gsub("_raw","",cell_types_all[[sample_list[i,1]]]),
                              img_path = paste(project_dir,"02_Cellranger/photos",sample_list[i,6],sep="/"),
    ...
    ggsave(plot3, width=5, height=5,dpi=600,units ="in",bg="white", filename= paste(spotlight_dir_sample,paste0("spatialDimPlot_",sample_list[i,1],"_",cell,".png"),sep="/"),limitsize = FALSE)
    ggsave(plot4, width=5, height=5,dpi=600,units ="in",bg="white", filename= paste(spotlight_dir_sample,paste0("spatial_scatterpie_",sample_list[i,1],"_",cell,".png"),sep="/"),limitsize = FALSE)
  ...
  decon_mtrx_sub2 <- decon_mtrx[[sample_list[i,1]]][, cell_types_all[[sample_list[i,1]]]]
  decon_mtrx_sub2 <- decon_mtrx_sub2[, colSums(decon_mtrx_sub2) > 0]

  # Compute correlation
  decon_cor <- cor(decon_mtrx_sub2)
  colnames(decon_cor) <- gsub("_raw","",colnames(decon_cor))
  rownames(decon_cor) <- gsub("_raw","",rownames(decon_cor))

  # Compute correlation P-value
  p.mat <- corrplot::cor.mtest(mat = decon_mtrx_sub2, conf.level = 0.95)

  # Visualize
  plot6=ggcorrplot::ggcorrplot(
        corr = decon_cor,
        p.mat = p.mat[[1]],
        hc.order = TRUE,
        type = "full",
        ...
        title = paste0("Predicted cell-cell proportion correlation of sample ",sample_list[i,1]),
  ...
  ggsave(plot6, width=10, height=10,dpi=600,units ="in",bg="white", filename= paste(spotlight_dir_sample,paste0("Predicted cell-cell proportion correlation of sample ",sample_list[i,1],".png"),sep="/"),limitsize = FALSE)
  saveRDS(spatial.ob[[sample_list[i,1]]],paste(rds_dir,paste0(sample_list[i,1],"_sc_celltype",".rds"),sep="/"))
...
# ###空间交互信息
#  colnames(decon_mtrx_sub2) <- gsub("_raw","",colnames(decon_mtrx_sub2))



# graph_ntw <- get_spatial_interaction_graph(decon_mtrx = decon_mtrx_sub2)
...
# grad_edge <- seq(0, max(round(edge_importance, 1)), 0.1)
...
#               dplyr::pull(color)
# # Open a png file
# png(paste(spotlight_dir_sample,paste0("Spatial_interactions_",sample_list[i,1],".png"),sep="/"),
# 		width=6,
# 		height=6,
# 		units="in",
# 		res=600,
...
if (1==2){
  ...
  ###############################开始
  ##细胞类型
  spatial.ob=readRDS("suerat_Rdata/Y18_sc_celltype.rds")
  ...
  ggsave(plot1, width=5, height=5,dpi=1000,units ="in",bg="white", filename="spatialDimPlot_18.png",limitsize = FALSE)
  
  ##基因表达
  spatial.ob=readRDS("suerat_Rdata/Y48_sc_celltype.rds")

  DefaultAssay(spatial.ob) <- "SCT"
  for (i in c("GSTA1","AMH","HSD17B1","APOA1","DCN","STAR","IL7R","KLRB1","TUBB8",
  "ZP3","FIGLA","STAG3","TYROBP","IFI30","CCL5","NKG7","ACTA2","MUSTN1","TM4SF1","VWF",
  "CDKN1A","CDKN1B","CDKN2A","CDKN2B","P53","IL6","IL8","IL1A","IL1B")){
    if( ! i %in% spatial.ob@assays$SCT@data@Dimnames[[1]]){next()}
    ...
    ggsave(plot2, width=5, height=5,dpi=1200,units ="in",bg="white", filename=paste0("genes_exp/Y48/SpatialFeaturePlot_",i,".png"),limitsize = FALSE)
  }
}