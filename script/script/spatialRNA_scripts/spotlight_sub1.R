...
option_list <- list(
       ...
       # make_option(c("-l", "--resolution"),type="double", help="resolution value for FindClusters, default %default",default="1.0"),
        ...
        make_option(c("-d", "--spatial_data"), help="rds for spatial data with celltypes, default %default",default="suerat_Rdata/Y48_sc_celltype.rds"),
...
spotlight_dir=paste(project_dir,"07_SpotLight_sub",sep="/")
...
for (v in 1:nrow(cells_cmbn)){
  ...
  curr.spotlight <- spotlight_deconvolution(
                                      se_sc = curr.sc, 
                                      counts_spatial = curr.spatial@assays$Spatial@counts,
                                      clust_vr = "new.ident",                                    # Variable in sc_seu containing the cell-type annotation
                                      cluster_markers = curr.cluster_markers,                    # Dataframe with the marker genes
  ...
  decon_mtrx <-  curr.spotlight[[2]]
  rownames(decon_mtrx) <- colnames(curr.spatial)
  colnames(decon_mtrx)[-ncol(decon_mtrx)]=mapper[as.numeric(gsub("celltype.","",colnames(decon_mtrx)[-ncol(decon_mtrx)]))]
  curr.ratio=decon_mtrx[colnames(spatial_tmp1)[spots_list],intersect(colnames(decon_mtrx),setdiff(mapper,colnames(cells_cmbn)))]   ### 注意，这里又去掉了增加的几个spot
  ...
  if( length(spots_list)==1 ){
    ...
    tmp_names=rownames(df)
    df=rbind(df,curr.df[[v]])
    rownames(df)=c(tmp_names,colnames(spatial_tmp1)[spots_list])
  }else{
    curr.df[[v]]=t(apply(curr.ratio,1,function(x){x/sum(x)}))*curr.spatial@meta.data[colnames(spatial_tmp1)[spots_list],opt$celltype]
    df=rbind(df,curr.df[[v]])
  }
...
if(nchar(opt$celltype2)>0 & (opt$celltype2 %in% colnames(spatial_tmp1@meta.data))){
  ...
  df_tmp$others=as.numeric(1-rowSums(df))
}
df_tmp <- df_tmp %>%
data.frame() %>%
...
for(j in 1:ncol(df)){
  ...
  ggsave(plot3, width=5, height=5,dpi=600,units ="in",bg="white", filename= paste(spotlight_dir_sample,paste0("spatialDimPlot_",sampleID,"_",cell,".png"),sep="/"),limitsize = FALSE)
}

# Remove cell types not predicted to be on the tissue
decon_mtrx_sub2 <- decon_mtrx[, colnames(df)]
...
plot6=ggcorrplot::ggcorrplot(
      ...
      title = paste0("Predicted cell-cell proportion correlation of sample ",sampleID),
...
ggsave(plot6, width=10, height=10,dpi=600,units ="in",bg="white", filename= paste(spotlight_dir_sample,paste0("Predicted cell-cell proportion correlation of sample ",sampleID,"_",opt$celltype,".png"),sep="/"),limitsize = FALSE)
saveRDS(spatial.ct,paste(rds_dir,paste0(sampleID,"_",opt$celltype,"_sc_subclusters",".rds"),sep="/"))
system(paste0("touch ",spotlight_dir_sample,"/spotlight_for_subclusters",sampleID,"_",opt$celltype,".finished"))
